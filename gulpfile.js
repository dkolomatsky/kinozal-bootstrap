/* Структура размещения файлов любого проэкта для работы gulp которая прописана в этом gulpfile.js !!!!!!!!!!!!!!!!!!
если поменять стуктуру и не поменять в настройках то работать ничего не будет
project/
  | index.html
  | assets/
      | scss/
      ...
      | js
          | main.js
          Структура размещения файлов для работы gulp !!!!!!!!!!!!!!!!!!

          установка gul глобально:
          запускаем команду npm install gulp cli -g
          потом запускаем команду npm install
          проверяем версию gulp -v
          Если в результате вы увидите версии
          CLI version x.x.x
          Local version 4.x.x - установка успешна.

          команда просмотра проэута в браузере gulp watch
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          обязательно в конце каждого сеанса работы закрываете отслеживание
          кнопка control + c
